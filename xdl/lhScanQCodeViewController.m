...
- (void)accordingQcode:(NSString *)str
{
    ...
    [SingleSDK postSdkBarcodePayWithPayModel:model
                                   AndAmount:@"1"
                             AndTotal_Amount:@"1"
                                 AndAuthCode:str
                               AndPayChannel:self.payChannel
                                  AndSubject:@"红板凳订单"
                               AndSelOrderNo:@"20170527153245"
                                AndGoods_tag:@"红板凳测试数据"
                                   AndAttach:@"附加数据" success:^(id data) {
                                       //成功
                                       NSLog(@"成功 : %@",data);
                                       UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"扫描结果" message:@"支付成功" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                                       [alertView show];
                                   } failure:^(NSError *error, NSString *m, NSString *message) {
                                       //失败
                                       NSLog(@"失败 : %@,%@,%@",error,m,message);
                                       UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"扫描结果" message:message delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                                       [alertView show];
                                   }];
